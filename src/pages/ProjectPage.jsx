...
const ProjectPage = () => {
  ...
  return (
    <div
      key={project.name}
      className="text-white flex flex-col justify-center w-full pt-20 appear relative"
    >
      <h1 className="flex justify-center text-6xl font-montserrat font-bold py-10 mb-10 border-b-2 border-white/30">
        {project.name}{" "}
      </h1>
      <div className="px-4">
      ...
      <div>
        <div className="flex flex-wrap items-start space-x-4 space-y-2 mb-10 px-2 justify-center">
          {project.languages.map((lang) => (
            <div
              key={lang}
              className="w-auto border-2 bg-white p-2 rounded-2xl text-black"
            >
              {lang}
            </div>
          ))}
      ...
      <h2 ref={ref} className={`flex justify-center text-4xl ${gradient}`}>
        {typedText}{" "}
        {!isDone && <span className={`text-4xl ${gradient} cursor`}>|</span>}
      </h2>
      ...
      {project.source && (
        <div className="flex flex-col items-center mb-10">
          <span className="text-lg font-semibold bg-gradient-to-br from-blue-500 to-green-500 bg-clip-text text-transparent mb-3">
            Source Code
          </span>
          <a
            href={project.source}
