...
const ProjectsDisplay = ({ gradient, home = true }) => {
  ...
  return (
    <div>
      <div ref={ref} className={`${gradient} text-5xl text-center mt-20 h-20`}>
        {" "}
        {typedText}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        {displayedProjects.map(
          ({ name, images, desc, languages, source }, index) => {
      ...
      {home && (
        <div className="flex justify-center mt-8">
          <Link
            to="/projects"
            className="px-6 py-3 bg-gradient-to-br from-blue-500 to-green-500 text-white rounded-full text-lg font-semibold shadow-md transition-transform duration-200 hover:scale-105"
          >
            View More
          </Link>
        </div>
