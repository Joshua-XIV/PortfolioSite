...
const ProjectDisplay = ({ name, image, desc, languages, source }) => {
  return (
    <div className="p-3 relative transition duration-300 hover:scale-105">
      <div
        className="gradient-border p-2 rounded-xl space-y-4 bg-white/5 text-white flex flex-col h-full"
        style={{ "--padding": "2px", "--inset": "0.75rem" }}
      >
        ...
        <div className="flex justify-between items-center pb-4 w-full h-auto">
          ...
          <Link
            to={`/projects/${name}`}
            className="border-1 border-white rounded-3xl p-2 relative shine overflow-hidden transition duration-300 hover:scale-105 flex space-x-2 items-center cursor-pointer"
          >
        ...
        <div className="flex flex-wrap space-x-2 space-y-2 mt-auto">
          {languages.map((lang, idx) => (
            <div
              className="bg-white text-black rounded p-1 w-fit h-fit text-[14px]"
              key={idx}
            >
              {lang}
            </div>
